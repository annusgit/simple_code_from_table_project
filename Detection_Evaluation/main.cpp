...
std::vector<std::string> regexSplit(const std::string &s, std::string rgx_str) {
    ...
    while (iter != end) {
        elems.push_back((basic_string<char> &&) *iter);
        ++iter;
    }
    return elems;
}
 ...
 * Second argument = GT directory (should contain ids.txt)
...
int main(int argc, char *argv[]) {
    ...
    for (auto i:groundTruthIds) {
        ...
        vector<Rect> groundTruthBoundingBoxes;
        cout << "Reading " << imagesDir + i + ".jpg" << endl;
        Mat image = imread(imagesDir + i + ".jpg", 1);
        Mat grayScale;
        cvtColor(image, grayScale, 1);
        Mat binaryImage;
